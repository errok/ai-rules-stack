---
description: Tune .claude/rules globs, descriptions, and the CLAUDE.md index so scoped context stays small and rules stay reusable
---

# Optimize rule attachment (globs + descriptions + CLAUDE.md index)
...
- every scoped rule carries a `globs:` line; the `PreToolUse` hook
  ...
  matching file (once per session);
- the **Domain rules** table in `CLAUDE.md` mirrors the same mapping as reference
  and as a fallback when hooks are disabled.

Goal: the always-on set stays minimal, each scoped rule has a tight, accurate
`globs:` and a keyword-rich `description`, and the `CLAUDE.md` table matches the
files on disk.
...
2. Flag overly broad globs, empty `description`s, and rows in the `CLAUDE.md`
   table that point at a missing/renamed file (or files missing from the table).
3. For each rule:
...
4. Rebuild the **Domain rules** table in `CLAUDE.md` from the files on disk
   (glob, path, one-liner), and confirm the `@`-import list is exactly the
   always-on set.
...
- Concrete edits to rule frontmatter/body and to the `CLAUDE.md` index.
- Short summary: which globs changed, which rules were merged/split, what was
  generalised, and any `CLAUDE.md` rows added/removed.
