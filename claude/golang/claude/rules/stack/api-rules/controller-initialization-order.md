...
- Services may keep taking their connection at construction (`database.GetDB()`), because the
  constructor now runs after the connection exists. Per-request state (the context, and the
  transaction it carries) still goes through `ctx`, never through the struct.
