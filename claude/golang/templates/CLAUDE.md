...
Everything above is the **generic Go backend stack**, shared with every project built on it —
...
- Generic rules come from the `ai-rules-stack` submodule (`sub-modules/ai-rules-stack/`, stack
  `golang`), linked into `.claude/` and `.cursor/`. Never edit them here: change them in the
