...
**Repos:** same workflow for a **client app** (e.g. TypeScript under `src/`) and a **Go API** (e.g. code under `app/`). Work from the **target repository root**; path examples below are illustrative—**globs must match that repo’s tree**.
...
1. **Tight globs for narrow domains** — smallest subtree where the rule should auto-attach.
   ...
   - **API (Go):** e.g. `app/services/**`, `app/controllers/**`, `app/router/**` — avoid blanket `app/**/*.go` unless the rule is universal for that repo.
...
3. **Glob YAML** — in frontmatter, write `globs` **without** surrounding double quotes (prefer one comma-separated line).
   - **Correct:** `globs: src/services/**` or `globs: app/controllers/**/*.go,app/router/**`
